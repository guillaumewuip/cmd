...
function AsyncIframeWrapper({ children }: PropsWithChildren<{}>): JSX.Element {
...
async function fetchBandcampTrackId(href: string) {
  const response = await fetch(
    `/api/bandcamp/track?url=${encodeURIComponent(href)}`
  );
...
export function Bandcamp({ album, track }: { album?: string; track: string }) {
...
async function fetchSoundcloudTrackId(href: string) {
  const response = await fetch(
    `/api/soundcloud/track?url=${encodeURIComponent(href)}`
  );
...
export function Soundcloud({ track }: { track: string }) {
...
export function isEmbedableLink(href: string) {
  return isYoutubeLink(href) || isBandcampLink(href) || isSoundcloudLink(href);
}
