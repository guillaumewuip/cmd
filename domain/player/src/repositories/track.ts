...
function updateTrack(trackId: string, modifier: (track: Track.Track) => Track.Track): IO.IO<void> {
  return Store.write(Tracks.modifyIfNotEmpty(
    Tracks.modifyTrack(trackId, modifier)
  ))
}

function doIfSelectedTrack(callback: (track: Track.Initialized) => IO.IO<void> ) {
...
const aborted = doIfSelectedTrack(
  (track: Track.Initialized) => Store.write(Tracks.modifyIfNotEmpty(
    Tracks.modifyTrack(Track.id(track), () => Track.aborted(track)))
  )
)

const buffering = doIfSelectedTrack(
  (track: Track.Initialized) => Store.write(Tracks.modifyIfNotEmpty(
    Tracks.modifyTrack(Track.id(track), track => {
      if (!Track.isInteractive(track)) {
        return track
      }

      return Track.buffering(track)
    }))
  )
)

const positionUpdate = (getPosition: () => Position.Position) => doIfSelectedTrack(
  (track: Track.Initialized) => Store.write(Tracks.modifyIfNotEmpty(
    Tracks.modifyTrack(Track.id(track), track => {
      if (!Track.isInteractive(track)) {
        return track
      ...
      return Track.positionChanged(getPosition())(track)
    }))
  )
)

const durationUpdate = (duration: number) => doIfSelectedTrack(
  (track: Track.Initialized) => Store.write(Tracks.modifyIfNotEmpty(
    Tracks.modifyTrack(Track.id(track), track => {
      ...
      return Track.updateDuration(duration)(track)
    }))
  )
)

const playing = doIfSelectedTrack(
  (track: Track.Initialized) => Store.write(Tracks.modifyIfNotEmpty(
    Tracks.modifyTrack(Track.id(track), Track.fold<Track.Track>({
      Reserved: identity,
      Aborted: identity,
      Playing: identity,
...
const paused = doIfSelectedTrack(
  (track: Track.Track) => Store.write(Tracks.modifyIfNotEmpty(
    Tracks.modifyTrack(Track.id(track), Track.fold<Track.Track>({
      Reserved: identity,
      Aborted: identity,
      Paused: identity,
...
const ended = doIfSelectedTrack(
  (track: Track.Track) => pipe(
    Store.write(Tracks.modifyIfNotEmpty(
      tracks => {
...
export function reserve({id, title, weight}: { id: string, title: string, weight: number }): IO.IO<Track.Reserved> {
...
export function loadYoutube({
  ...
  return pipe(
    TaskEither.tryCatch(
      (): Promise<Source.TrackSource> => new Promise((resolve, reject) => {
        try {
          window.YT.ready(() => {
            ...
            setInterval(() => {
              if ("getPlayerState" in player && player.getPlayerState() === window.YT.PlayerState.PLAYING) {
                const time = player.getCurrentTime()
                const duration = player.getDuration()


                const position = Position.create(time / duration)
                return positionUpdate(() => position)(trackId)()
              }
            }, 1000);
    ...
    TaskEither.chainFirstIOK((source) => updateTrack(
      Track.id(track),
      () => Track.initialized({ source })(track)
    )),
    TaskEither.map(() => {}),
  )
}
...
interface SoundcloudWidget {
  ...
  bind(
    ...
    callback: (state: { relativePosition: number, loadProgress: number, currentPosition: number }) => void
  ): void
  bind(
  ...
  return pipe(
    IOEither.tryCatch(
      () => {
        ...
        widget.bind(window.SC.Widget.Events.PLAY_PROGRESS, throttle(({ relativePosition, currentPosition }) => {
          positionUpdate(
            () => Position.create(relativePosition)
          )(trackId)()

          if (relativePosition) {
            // millis to seconds conversion
            durationUpdate(currentPosition / 1000 / relativePosition)(trackId)()
          }
        }, 1000))
    ...
    IOEither.chainFirstIOK((source) => updateTrack(
      Track.id(track),
      () => Track.initialized({ source })(track)
    )),
    IOEither.map(() => {}),
  ...
  return pipe(
    IOEither.tryCatch(
      () => {
        ...
        audio.addEventListener('timeupdate', () => positionUpdate(
